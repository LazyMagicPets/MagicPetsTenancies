...
# SystemSuffix: "yourSuffix-here-496a-bd90-27f541ff523b"
# This Suffix is the Suffix of the system that the asssets belongs to. If you are only doing front end 
# work, this Suffix should be provided to you by the back-end team. If you are doing back-end work,
# then review the Service project AWSTemplates folder for more setup information.
# Note the TenantKey parameter. This is the name of the tenancy that the configuration is for.

param(
	[string]$TenantKey="system",
	[string]$SubtenantKey="",
	[string]$Suffix
)
...
$filePath = "../../systemconfig.yaml"
if(-not (Test-Path $filePath))
{
	Write-Host "systemconfig.yaml file missing. See the Service/AWSTemplate folder for more information."
	exit
}

$config = Get-Content -Path $filePath | ConvertFrom-Yaml
$SystemSuffix = $config.SystemSuffix
if(-not $Suffix.HasValue) {
	$Suffix = $SystemSuffix
}

$SystemKey = $config.SystemKey
$bucketName = $SystemKey + "---assets-" + $Suffix
...
foreach($assetLanguage in $assetLanguages) {
	...
	foreach($assetGroup in $assetGroups) {
		...
		foreach ($asset in $assets) {
			...
			$relativePath = $TenantKey + "/" + $assetLanguageName + "/" + $relativePath
			$manifest += @{
